		...
		populatePayPalForm: function() {
			...
			if( self.$paypalForm.length ) {
				...
				for( var i = 0; i < cartItems.length; ++i ) {
					...
					insertBefore( "#paypal-btn" );
					$( "<div/>" ).html( "<input type='hidden' name='woIce" + n + "' value=' " + ice + "'/>" ).
					insertBefore( "#paypal-btn" );
...
//stores each item in an array as a seperate entity
					for( var i = 0; i < items.length; ++i ) {
						var item = items[i];
						var product = item.product;
            ...
            // var topping = item.topping;
				...
				if( cartItems.length > 0 ) {
					...
					for( var j = 0; j < cartItems.length; ++j ) {
						...
						var cartIce = cartItem.ilvl;
						var cartHTML = "<tr><td class='pname'>" + cartProduct + "</td>" + "<td class='pqty'>" + cartQty + "</td>"+ "<td class='pilvl'>" + cartIce + "</td>"+ "<td class='pprice'>" + cartPrice + "</td></tr>";
		  ...
		  if( self.$updateCartBtn.length ) {
			self.$updateCartBtn.on( "click", function() {
				...
				$rows.each(function() {
					...
					var pIce = self._convertString( $row.find( ".pIce > .ilvl" ).val() );
		...
		handleAddToCartForm: function() {
			...
			self.$formAddToCart.each(function() {
				...
				/*This pulls directly from the HTML*/
				var price = self._convertString( $product.data( "price" ) );
				var name =  $product.data( "name" );
				// var toppings = $product.data("toppings");

				$form.on( "submit", function() {
					/*this variables are user determined*/
					var qty = self._convertString( $form.find( ".qty" ).val() );
					var ilvl = self._convertString( $form.find( ".ilvl" ).val() );
